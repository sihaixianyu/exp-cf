...
if __name__ == '__main__':
    ...
    util.sep_print(config['data_name'], desc='Dataset Name')

    data_dir = path.join(root_dir, 'data/', config['data_name'])
    train_arr, test_arr, ui_csr_mat, neg_dict, info_dict = util.load_data(data_dir)
    dataset = Dataset(train_arr, test_arr, ui_csr_mat, neg_dict, info_dict)

    dataset.set_sample_method(config['sample_method'])
    util.sep_print(config['sample_method'], desc='Sample Method')
    ...
    if model_name == 'mf':
        ...
        model = EMF(dataset, model_config, data_dir)
        optimizer = Adam(model.parameters(), lr=model_config['lr'], weight_decay=model_config['weight_decay'])
    elif model_name == 'egcn':
        model = EGCN(dataset, model_config, data_dir)
        optimizer = Adam(model.parameters(), lr=model_config['lr'])
    else:
    ...
    model_path = path.join(model_dir, '{}_{}.pth'.format(config['data_name'], config['model_name'].lower()))
