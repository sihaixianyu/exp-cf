...
class CEMF(BaseModel):
    ...
    def forward(self, batch_data):
        ...
        pos_exp_reg = (1 / 2) * (pos_emb_diffs * self.ui_exp_tsr[users, pos_items]).norm().pow(2) / float(len(users))
        neg_exp_reg = (1 / 2) * (neg_emb_diffs * self.ui_exp_tsr[users, neg_items]).norm().pow(2) / float(len(users))
    ...
    def get_model_path(self, model_dir: str):
        return path.join(model_dir, '{}_ld{}_n{}_b{}_g{}.pth'.format(self.model_name,
                                                                     self.latent_dim,
                                                                     self.neighbor_num,
                                                                     self.beta,
                                                                     self.gamma))
